...
### SEC-1 🔴 Child role is not enforced on the backend
...
### SEC-2 🟠 Last-admin lockout via role change
...
### SEC-3 🟠 Internal error strings leaked to clients
...
### BUG-1 🔴 Auto-learned shelf order never works
...
### BUG-2 🔴 Deleting a list or store fails silently
...
### BUG-3 🟠 Sender receives its own broadcast (X-Conn-ID is dead code)
...
### BUG-4 🟠 Scanned Open Food Facts products are never persisted
**Where:** `backend/handlers/products.go` `GetByBarcode`,
...
### BUG-5 🟠 Offline-synced items lose category/store metadata
...
### BUG-6 🟡 pt-BR suggestions show the German category name
...
### BUG-7 🟡 Repeat purchases of the same item aren't recorded
...
### BUG-8 🟡 Unguarded `JSON.parse` in the WebSocket client
...
### BUG-9 🟡 Shelf-order default position mismatch (frontend 999 vs backend 9999)
...
### TD-1 🟠 i18n convention not followed (hardcoded German strings)
...
### TD-2 🟠 Two divergent client-side event reducers
...
### TD-3 🟡 `SetMaxOpenConns(1)` serializes all DB access
...
### TD-4 🟡 Package-global DB for shelf learning
...
### TD-5 🟡 Debug `console.log` left in production code
...
### TD-9 🟡 OFF integration is a stub
...
### Milestone A — correctness & data safety (do first)
...
### Milestone E — maintainability
17. **TD-3** DB connection pooling (coordinate with BUG-1 fix). *(M)*
18. **TD-4** inject DB instead of global. *(S)*
19. **TD-5** remove debug logging. *(S)*
20. **TD-7** remove dead config/code. *(S)*
21. **TD-8** add tests for events/suggestions/access/sync. *(L)*
22. **BUG-9** unify shelf-order default constant. *(S)*

### Suggested quick-win first PR
`SEC-2`, `BUG-6`, `BUG-7`, `BUG-8`, `SEC-3`, `TD-5`, `TD-7` are all **S** and
independent — a single low-risk cleanup PR knocks out seven findings.

---

## 5. Summary table

| ID | Sev | Area | One-liner | Effort |
|----|-----|------|-----------|--------|
| SEC-1 | 🔴 | Security | Child role not enforced on events endpoint | M |
| SEC-2 | 🟠 | Security | Last-admin can be demoted → lockout | S |
| SEC-3 | 🟠 | Security | Internal error text leaked to client | S |
| SEC-4 | 🟡 | Security | Barcode param unvalidated into outbound URL | S |
| SEC-5 | 🟡 | Security | No body-size cap / rate limiting | M |
| SEC-6 | 🟡 | Security | Member directory visible to all roles | S |
| BUG-1 | 🔴 | Bug | Auto shelf-order learning never works | M |
| BUG-2 | 🔴 | Bug | List/store delete fails silently (FK, swallowed error) | M |
| BUG-3 | 🟠 | Bug | X-Conn-ID echo-exclusion is dead code | M |
| BUG-4 | 🟠 | Bug | OFF scanned products never persisted | M |
| BUG-5 | 🟠 | Bug | Offline sync drops item category/store | M |
| BUG-6 | 🟡 | Bug | pt-BR category names show German | S |
| BUG-7 | 🟡 | Bug | Repeat purchases not recorded | S |
| BUG-8 | 🟡 | Bug | Unguarded JSON.parse in WS client | S |
| BUG-9 | 🟡 | Bug | Shelf-order default 999 vs 9999 mismatch | S |
| TD-1 | 🟠 | Debt | Hardcoded German strings (i18n unused) | L |
| TD-2 | 🟠 | Debt | Two divergent event reducers | M |
| TD-3 | 🟡 | Debt | MaxOpenConns(1) serializes all reads | M |
| TD-4 | 🟡 | Debt | Global DB for shelf learning | S |
| TD-5 | 🟡 | Debt | Debug console.log in prod | S |
| TD-6 | 🟡 | Debt | Ignored write errors | M |
| TD-7 | 🟡 | Debt | Dead config/code (LogLevel, MaxUploadSize, sql hack) | S |
| TD-8 | 🟡 | Debt | Thin test coverage | L |
| TD-9 | 🟡 | Debt | OFF integration is a stub | M |
</content>
</invoke>
